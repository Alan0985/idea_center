...
const IdeaCard = ({ idea, handleTagClick, handleEdit, handleDelete }) => {
  ...
  return (
    <div className="idea_card">
      <div className="flex justfiy_between items_start gap-5">
        <div className="flex-1 flex justify-start items-center gap-3 cursor-pointer">
          <Image
            className="rounded-full object-contain"
            src={
              idea.creator.image === undefined
      ...
      <p className="my-4 font-satoshi text-sm text-gray-700">{idea.idea}</p>
      <p
        className="font-inter text-sm blue_gradient cursor-pointer"
        onClick={() => handleTagClick && handleTagClick(idea.tag)}
      >
        {idea.tag}
      </p>
